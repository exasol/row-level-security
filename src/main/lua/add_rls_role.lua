...
if not BASE.role_id_is_valid(role_id) then
    error('Invalid role id. Role id must be between 1 and 63.')
end
_role_id = BASE.get_role_id_by_name(role_name)
_role_name = BASE.get_role_name_by_id(role_id)
if _role_id ~= nil then
    error('Role name "' .. role_name .. '" already exists (role id ' .. _role_id .. ').')
end
if _role_name ~= nil then
    error('Role id ' .. role_id .. ' already exists (role name "' .. _role_name .. '").')
end
